...
Modified Blowfish Algorithm For Sify Authentication
By Liet-Kynes

* Modified P-Arrays and S-Boxes
* Added string encryption method (with byte reversal)
...
static const unsigned long ORIG_P[16 + 2] = {
        ...
        0xC0AC29B7L, 0xC87C50DDL, 0x3F84D5B5L, 0xB5470917L, //0xC97
        0x9216D5D0L, 0x8979FB1BL //0x9216d5d9
};

static const unsigned long ORIG_S[4][256] = {
    {   0xD1310BA6L, 0x98DFB5ACL, 0x2FFD72DBL, 0xD01ADFB7L,
        0xF8E1AFEDL, 0x6A267E96L, 0xBA7C9045L, 0xF12C7F99L,
        0x24A19947L, 0xB3916CF7L, 0x0801F2E2L, 0x858EFC16L,
        0x636920D8L, 0x71574E69L, 0xA458FEA3L, 0xF4933D7EL,
        0x0D95748FL, 0x728EB658L, 0x718BCD58L, 0x82154AEEL,
        0x7B54A41DL, 0xC25A59B5L, 0x9C30D539L, 0x2AF26013L,
        0xC5D1B023L, 0x286085F0L, 0xCA417918L, 0xB8DB38EFL,
        0x8979DCB0L, 0x603A180EL, 0x6C9E0E8BL, 0xC01E8A3EL,
...
Quick hack method to encrypt strings
By Liet-Kynes

Reverses byte ordering as in the Sify Auth
TODO: Optimize,optimize,optimize


====================================================*/

char* encryptString(const char* szString, const char* pKey) {

  unsigned long Xl,Xr,swp;
  int origLen = strlen(szString);
  int len = origLen%8;
  len=len==0?origLen:(8-len+origLen);
  char* pMem = (char*)malloc(len);
  char* pCipher = (char*)malloc(len*2+1);
  pCipher[len*2]=0;

  memcpy(pMem,szString,origLen);
  memset(pMem+origLen,0,len-origLen);

  BLOWFISH_CTX ctx;

  Blowfish_Init (&ctx, (unsigned char*)pKey, strlen(pKey));

  int i=0;
  for(;len>0;i+=8) {
    Xl=Xr=0;
    swp = *((unsigned long*)(pMem+i));
    Xl = (swp<<24)|(swp>>24)|((swp&0xFF0000)>>8)|((swp&0xFF00)<<8);
    swp = *((unsigned long*)(pMem+i+4));
    Xr= (swp<<24)|(swp>>24)|((swp&0xFF0000)>>8)|((swp&0xFF00)<<8);

    Blowfish_Encrypt(&ctx,&Xl,&Xr);
    sprintf((pCipher+i*2),"%08x%08x",Xl,Xr);
    len-=8;
  }
  return pCipher;
